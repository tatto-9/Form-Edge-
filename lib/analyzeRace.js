...
// Used by both api/predict.js (one race) and api/tipsheet.js (every race
// at a venue).
//
// No live odds source is wired in right now — Betfair access isn't set
// up. Odds are always 'n/a' until that's added back.
...
async function analyzeRace({ raceId, manualRunners, track, distance, going, raceName }) {
  ...
  if (raceId) {
    ...
    runners = fieldRunners
      ...
      .map(r => ({
        ...
        odds: 'n/a', // No live odds source wired in right now
      }));
  ...
  const prompt = `You are a horse racing form analyst. Analyze this race and rank the runners from most to least likely to win, based only on the data given. Be realistic — don't invent facts not provided. No live market odds are available — reason from form and handicapping data only, not price.
