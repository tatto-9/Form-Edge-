...
- `public/index.html` — the page you see (race picker, manual entry, results, tip sheet, My Picks)
- `api/predict.js` — analyzes a single race (Punting Form + Betfair, or manual entry) using Claude
- `lib/analyzeRace.js` — the shared analysis logic used by predict.js; the tip sheet calls this once per race, from the browser, rather than a server-side loop
...
- `lib/raceTime.js` — handles Australian-timezone date/time logic correctly (the server itself runs in UTC)
- `vercel.json` — requests extended execution time for single-race predictions
...
**Betfair:** a Developer App Key (Delayed tier is fine to start) plus your
normal Betfair username/password — see the earlier walkthrough for
creating this via api.developer.betfair.com. If these aren't set up yet,
the tool still works fully; odds just show as n/a until then.
...
2. Settings → Environment Variables, add:
   - `ANTHROPIC_API_KEY`
   - `PUNTINGFORM_API_KEY`
   - `BETFAIR_APP_KEY`, `BETFAIR_USERNAME`, `BETFAIR_PASSWORD` (once you have them)
